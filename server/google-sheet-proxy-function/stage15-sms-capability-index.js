...
function assertRequestAllowed(req) {
  ...
  const standaloneActions = new Set(["admin_login", "admin_erp_login_check", "admin_erp_member_lookup", "admin_release_v2_status", "admin_release_v2_rollback", "admin_release_v2_browser_gate", "admin_product_discovery_status", "admin_product_discovery_shadow_compare", "admin_product_discovery_browser_gate", "home_stats", "secret_tour_goods_detail", "secret_tour_flight_schedule", "secret_tour_goods_list", "secret_tour_goods_events"]);
...
function assertServiceRole(req) {
  ...
  const isAllowed = GOLFJOIN_SERVICE_ROLE === "main"
    ? !["quote_generate", "send_application_notifications", "aligo_sms_capability"].includes(action)
    : GOLFJOIN_SERVICE_ROLE === "quote"
      ? ["quote_generate", "quote_pdf", "quote_view"].includes(action)
      : GOLFJOIN_SERVICE_ROLE === "aligo"
        ? ["send_application_notifications", "aligo_sms_capability"].includes(action)
        : false;
...
function buildBasePublicParticipantSummary(schedule = {}) {
  ...
  const creatorPeople = schedule.isAdminRecommendedSchedule
    ? 0
...
async function refreshSecretTourProducts(req, res) {
  ...
  res.status(200).json({
    ...
    productDiscovery: {
      discoveryRevision: storagePublication.productDiscovery.publication.discoveryRevision,
      browserReadEnabled: storagePublication.productDiscovery.root.payload.browserReadEnabled === true,
      ...
      rootUpdatedLast: storagePublication.productDiscovery.rootUpdatedLast === true
    },
    productFamily: {
      catalogRevision: productState.catalogRevision,
      analysisRevision: productState.analysisRevision,
...
exports.proxyGoogleSheet = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && ["send_application_notifications", "aligo_sms_capability"].includes(asText(req.query?.action))) {
