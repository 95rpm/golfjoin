...
function assertRequestAllowed(req) {
  ...
  const standaloneActions = new Set(["admin_login", "admin_erp_login_check", "admin_erp_member_lookup", "admin_release_v2_status", "admin_release_v2_rollback", "admin_release_v2_browser_gate", "home_stats", "secret_tour_goods_detail", "secret_tour_flight_schedule", "secret_tour_goods_list", "secret_tour_goods_events"]);
...
async function proxyPost(req, res) {
  ...
  if (source === "join_apply" && GOOGLE_SHEET_ID) {
    try {
      ...
      if (isJoinScheduleFullError(error) || error?.writeCommitted || error?.code === "participant_summary_sync_failed") throw error;
